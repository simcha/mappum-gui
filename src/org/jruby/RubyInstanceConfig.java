...
public class RubyInstanceConfig {
	...
	/**
	 * The version to use for generated classes. Set to current JVM version by
	 * default
	 */
	 ...
	 */
	public static final int CHAINED_COMPILE_LINE_COUNT = SafePropertyAccessor
			.getInt("jruby.compile.chainsize",
	...
	private boolean objectSpaceEnabled = SafePropertyAccessor.getBoolean(
			"jruby.objectspace.enabled", false);
	...
	private ClassLoader loader = contextLoader == null ? RubyInstanceConfig.class
			.getClassLoader()
			: contextLoader;
	...
	// This property is a Boolean, to allow three values, so it can match MRI's
	// nil, false and true
	...
	public static final boolean PEEPHOLE_OPTZ = SafePropertyAccessor
			.getBoolean("jruby.peephole.optz", true);
	public static boolean FASTEST_COMPILE_ENABLED = SafePropertyAccessor
			.getBoolean("jruby.compile.fastest");
	public static boolean FASTOPS_COMPILE_ENABLED = FASTEST_COMPILE_ENABLED
			|| SafePropertyAccessor.getBoolean("jruby.compile.fastops");
	public static boolean FRAMELESS_COMPILE_ENABLED = FASTEST_COMPILE_ENABLED
			|| SafePropertyAccessor.getBoolean("jruby.compile.frameless");
	public static boolean POSITIONLESS_COMPILE_ENABLED = FASTEST_COMPILE_ENABLED
			|| SafePropertyAccessor.getBoolean("jruby.compile.positionless");
	public static boolean THREADLESS_COMPILE_ENABLED = FASTEST_COMPILE_ENABLED
			|| SafePropertyAccessor.getBoolean("jruby.compile.threadless");
	public static boolean FASTCASE_COMPILE_ENABLED = SafePropertyAccessor
			.getBoolean("jruby.compile.fastcase");
	public static boolean FASTSEND_COMPILE_ENABLED = SafePropertyAccessor
			.getBoolean("jruby.compile.fastsend");
	public static boolean LAZYHANDLES_COMPILE = SafePropertyAccessor
			.getBoolean("jruby.compile.lazyHandles", false);
	public static final boolean FORK_ENABLED = SafePropertyAccessor
			.getBoolean("jruby.fork.enabled");
	public static final boolean POOLING_ENABLED = SafePropertyAccessor
			.getBoolean("jruby.thread.pool.enabled");
	public static final int POOL_MAX = SafePropertyAccessor.getInt(
			"jruby.thread.pool.max", Integer.MAX_VALUE);
	public static final int POOL_MIN = SafePropertyAccessor.getInt(
			"jruby.thread.pool.min", 0);
	public static final int POOL_TTL = SafePropertyAccessor.getInt(
			"jruby.thread.pool.ttl", 60);

	public static final boolean NATIVE_NET_PROTOCOL = SafePropertyAccessor
			.getBoolean("jruby.native.net.protocol", false);

	public static boolean FULL_TRACE_ENABLED = SafePropertyAccessor.getBoolean(
			"jruby.debug.fullTrace", false);

	public static final String COMPILE_EXCLUDE = SafePropertyAccessor
			.getProperty("jruby.jit.exclude");
	...
	public RubyInstanceConfig() {
		...
		samplingEnabled = SafePropertyAccessor.getBoolean(
				"jruby.sampling.enabled", false);
		String compatString = SafePropertyAccessor.getProperty(
				"jruby.compat.version", "RUBY1_8");
		if (compatString.equalsIgnoreCase("RUBY1_8")) {
			...
			error.println("Compatibility version `" + compatString
		...
		if (Ruby.isSecurityRestricted()) {
			...
			String threshold = SafePropertyAccessor
					.getProperty("jruby.jit.threshold");
			String max = SafePropertyAccessor.getProperty("jruby.jit.max");
			String maxSize = SafePropertyAccessor
					.getProperty("jruby.jit.maxsize");
			...
			managementEnabled = SafePropertyAccessor.getBoolean(
					"jruby.management.enabled", true);
			runRubyInProcess = SafePropertyAccessor.getBoolean(
					"jruby.launch.inproc", true);
			boolean jitProperty = SafePropertyAccessor
			...
			if (jitProperty) {
				...
				compileMode = SafePropertyAccessor
						.getBoolean("jruby.jit.enabled") ? CompileMode.JIT
						: CompileMode.OFF;
			} else {
				String jitModeProperty = SafePropertyAccessor.getProperty(
						"jruby.compile.mode", "JIT");
			...
			jitLoggingVerbose = SafePropertyAccessor
					.getBoolean("jruby.jit.logging.verbose");
			String logEvery = SafePropertyAccessor
					.getProperty("jruby.jit.logEvery");
			jitLogEvery = logEvery == null ? 0 : Integer.parseInt(logEvery);
			jitThreshold = threshold == null ? JIT_THRESHOLD : Integer
					.parseInt(threshold);
			jitMax = max == null ? JIT_MAX_METHODS_LIMIT : Integer
					.parseInt(max);
			jitMaxSize = maxSize == null ? JIT_MAX_SIZE_LIMIT : Integer
					.parseInt(maxSize);
		}
	...
	public String getExtendedHelp() {
		...
		sb
				...
				.append(
						"  -Y              compile a Ruby script into YARV bytecodes and run this (EXPERIMENTAL)\n")
				.append(
						"  -R              read a Rubinius-compiled Ruby script and run that (EXPERIMENTAL)\n");
	...
	public String getPropertyHelp() {
		...
		sb
				...
				.append(
						"       Set the number of lines at which compiled bodies are \"chained\". Default is "
								+ CHAINED_COMPILE_LINE_COUNT_DEFAULT + "\n")
	...
	public String getVersionString() {
		...
		String fullVersion = String.format(
				...
				Constants.VERSION, ver, patchlevel, Constants.COMPILE_DATE,
				Constants.REVISION, SafePropertyAccessor.getProperty("os.arch",
						"unknown"));
	...
	public void setLoader(ClassLoader loader) {
		...
		if (this.loader != loader) {
			this.classCache = new ClassCache<Script>(loader, this.classCache
					.getMax());
		}
		this.loader = loader;
	}
	...
	private String verifyHome(String home) {
		...
		if (!home.startsWith("file:")) {
			...
			if (!f.exists()) {
				System.err.println("Warning: JRuby home \"" + f
	...
	private class ArgumentProcessor {
		...
		public void processArguments() {
			while (argumentIndex < arguments.length
					&& isInterpreterArgument(arguments[argumentIndex])) {
				processArgument();
				argumentIndex++;
			}
			...
			if (!hasInlineScript && scriptFileName == null) {
				if (argumentIndex < arguments.length) {
					setScriptFileName(arguments[argumentIndex]); // consume the
					// file name
					argumentIndex++;
				}
			}
			...
			FOR: for (characterIndex = 1; characterIndex < argument.length(); characterIndex++) {
				...
				case '0': {
					...
					if (null == temp) {
						...
						recordSeparator = "\uFFFF"; // Specify something that
						// can't separate
					} else {
						try {
							...
							MainExitException mee = new MainExitException(
									1,
				...
				case 'I':
					...
					break FOR;
				case 'K':
					// FIXME: No argument seems to work for -K in MRI plus this
					// should not
					// siphon off additional args 'jruby -K ~/scripts/foo'. Also
					// better error
					// processing.
				...
				case '-':
					if (argument.equals("--command")
						...
						if (argument.equals("--")) {
							...
							// Usage: ruby [switches] [--] [programfile]
							// [arguments])
							endOfArguments = true;
							break;
						}
				...
				default:
					throw new MainExitException(1, "jruby: unknown option "
							+ argument);
				}
			}
		}
		...
		private String resolveScript(String scriptName) {
			...
			if (path != null) {
				String[] paths = path.split(System
	...
	public InputStream getScriptSource() {
		try {
			// KCode.NONE is used because KCODE does not affect parse in Ruby
			// 1.8
			// if Ruby 2.0 encoding pragmas are implemented, this will need to
			// change
			if (hasInlineScript) {
				...
				File file = JRubyFile.create(getCurrentDirectory(),
						getScriptFileName());
